...
**************************************************************************************************************************************************************************************************************************************************************************************************************
 - Windows 10 22H2 or Windows 11 environment to run the script.
 ...
 - Windows ADK / WinPE installed, or allow the script to download a compatible version automatically.  Current Windows 11 media (including 23H2/24H2) will use the latest supported ADK / WinPE when required.
...
# How to use (general)
 - Acquire a Windows 10 22H2 or Windows 11 21H2/22H2/23H2/24H2 ISO that includes Professional or Enterprise images
 ...
 - To create a Windows 11 Professional image:
    X:\SurfaceDeploymentAccelerator\CreateSurfaceWindowsImage.ps1 -ISO "X:\en-us_windows_11_business_editions_version_24h2_x64.iso" -OSSKU Pro -DestinationFolder C:\Output -Device SurfacePro10

 - To create a Windows Enterprise image for use on a Surface Hub 2:
    X:\SurfaceDeploymentAccelerator\CreateSurfaceWindowsImage.ps1 -ISO "X:\en-us_windows_11_business_editions_version_24h2_x64.iso" -OSSKU Enterprise -DestinationFolder C:\Output -Device SurfaceHub2

- To create an image WITHOUT any additional patches downloaded/injected for a Surface Laptop 6:
    X:\SurfaceDeploymentAccelerator\CreateSurfaceWindowsImage.ps1 -ISO "X:\en-us_windows_11_business_editions_version_24h2_x64.iso" -OSSKU Pro -DestinationFolder C:\Output -Device SurfaceLaptop6 -ServicingStack $false -CumulativeUpdate $false -CumulativeDotNetUpdate $false -AdobeFlashUpdate $false

 - To create an image WITHOUT DotNet 3.5 or Office 365 C2R installed for a Surface Go 4:
    X:\SurfaceDeploymentAccelerator\CreateSurfaceWindowsImage.ps1 -ISO "X:\en-us_windows_11_business_editions_version_24h2_x64.iso" -OSSKU Pro -DestinationFolder C:\Output -Device SurfaceGo4 -DotNet35 $false -Office365 $false
...
***********************************************************************************************************************************************************************
 ...
 - The script is currently aligned to Windows 10 22H2 and Windows 11 21H2/22H2/23H2/24H2 source media.  Older releases may still work, but they are no longer the primary validation target.
 - For newer/current Surface devices that are not yet covered by a curated WinPE driver list, the script will fall back to injecting all extracted drivers into boot media.  You can also use -UseLocalDriverPath with a locally downloaded Surface driver package when preferred.
...
The parameters that are supported to configure for the script are as follows:
 ...
 -AdobeFlashUpdate:           Include latest Adobe Flash Player Security update, True or False.  True is the default.  This is a legacy Windows 10-era option and is typically unnecessary for current Windows 11 media.
 
 -Office365:                  Download and install the latest monthly C2R installation of Office 365, True or False.  True is the default.
 
 -Device:                     Enter Surface device type to download and inject latest drivers for.  Possible values: SurfacePro4, SurfacePro5, SurfacePro6, SurfacePro7, SurfacePro7Plus, SurfacePro8, SurfacePro9Intel, SurfacePro10, SurfacePro11, SurfaceLaptop, SurfaceLaptop2, SurfaceLaptop3Intel, SurfaceLaptop3AMD, SurfaceLaptop4Intel, SurfaceLaptop4AMD, SurfaceLaptop5, SurfaceLaptop6, SurfaceLaptop7, SurfaceLaptopGo, SurfaceLaptopStudio, SurfaceLaptopStudio2, SurfaceBook, SurfaceBook2, SurfaceBook3, SurfaceStudio, SurfaceStudio2, SurfaceGo, SurfaceGoLTE, SurfaceGo2, SurfaceGo3, SurfaceGo4, SurfaceHub2, and Custom.  If this parameter is not specified, SurfacePro8 is used.
